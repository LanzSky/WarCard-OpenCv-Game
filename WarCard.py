...
def play_round(human_card, bot_card):
    ...
    human_card_last = human_card[-1].split(' of ')
    bot_card_last = bot_card[-1].split(' of ')

    human_card_value = values.get(human_card_last[0], 0)
    bot_card_value = values.get(bot_card_last[0], 0)

    if human_card_value > bot_card_value:
        return "Human wins", human_card_last, bot_card_last
    elif human_card_value < bot_card_value:
        return "Bot wins", human_card_last, bot_card_last
    else:
        return "It's a tie", human_card_last, bot_card_last
...
while cam_quit == 0:
    ...
    if len(human_card) > 0 and len(bot_card) > 0:
